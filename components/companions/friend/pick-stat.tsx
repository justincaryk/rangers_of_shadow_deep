...
export default function FriendBonusStat({ feat }: Props) {
  ...
  const { mutate: addMemberStat } = useStatsApi().createMemberStat
  const { mutate: removeMemberStat } = useStatsApi().deleteMemberStat
      ...
      {show && (
        <Card className='bg-black/50'>
          <div className='space-y-6'>
            ...
            {memberStats?.allMemberStats?.nodes
              .filter(stat => (feat.pickIds?.length ? feat.pickIds.includes(stat.id) : true))
              .map(saved => (
                <div key={saved.id}>
