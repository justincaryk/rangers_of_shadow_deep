export const sectionBaseStyles = {
  'border-b-2 border-transparent hover:border-slate-300 hover:border-b-2 hover:border-dashed block pb-6': true,
}

export const memberCardStyles = {
  wrapper: {
    'rounded shadow-lg': true,
  },
  mainOuter: {
    'p-2 flex justify-between rounded-t bg-orange-900': true,
  },
  mainInnerPrimary: {
    'text-xl font-bold': true,
  },
  mainInnerSecondary: {
    'whitespace-nowrap overflow-ellipsis uppercase font-roboto text-xs text-slate-400 font-bold': true,
  },
  mainImgWrapper: {
    '': true,
  },
  minorOuter: {
    'p-2 flex justify-between rounded-b bg-slate-500/30': true,
  },
}
